 ...
 * 
 * <p>Adapters bridge the gap between framework-specific test events and the
 * custom reporter's canonical data model. Each test framework (TestNG, JUnit, etc.)
 * should implement this interface to provide framework-specific mapping logic.</p>
 * 
 * <h2>Design Intent</h2>
 * <p>The Adapter interface serves <b>TWO distinct purposes</b>:</p>
 * <ol>
 *   <li><b>Framework Integration Layer:</b> Lifecycle methods ({@code onTestStart()},
 *       {@code onTestSuccess()}, etc.) are <b>hooks for framework adapters to implement</b>,
 *       NOT methods for end users to call directly. These methods are invoked BY the test
 *       framework (TestNG, JUnit, etc.) to notify the adapter of test lifecycle events.</li>
 *   <li><b>Test Code API:</b> Methods {@link #addArtifact(String, Artifact)} and
 *       {@link #addMetric(String, Metric)} are the <b>public API for test code</b> to attach
 *       screenshots, logs, performance metrics, etc. to tests during execution.</li>
 * </ol>
 * 
 * <p><b>IMPORTANT:</b> The Adapter is NOT a standalone test runner. Calling lifecycle methods
 * directly (e.g., {@code adapter.onTestSuccess("testName")}) does NOT execute tests or produce
 * a TestRun. Adapters must be registered as framework listeners to properly capture test results.</p>
 * 
 ...
 *   <li><b>Artifacts & Metrics:</b> Support adding screenshots, logs, HTTP data,
 *       and performance metrics to the current test context</li>
 *   <li><b>Deterministic Aggregation:</b> Ensure consistent ordering and aggregation
 *       of test results for reproducible reports</li>
 *   <li><b>Thread Safety:</b> Use concurrent collections for parallel test execution</li>
 * </ul>
 * 
...
public interface Adapter {
     ...
     * 
     * <p><b>Framework Hook:</b> This method is invoked BY the test framework
     * (TestNG, JUnit, etc.) when a suite begins, NOT called directly by test code.
     * Implementations should initialize suite-level state and tracking.
     * This method may be called multiple times for different suites in the
     * same test run.</p>
     * 
     ...
     * 
     * <p><b>Framework Hook:</b> This method is invoked BY the test framework
     * (TestNG, JUnit, etc.) when a suite completes, NOT called directly by test code.
     * Implementations should finalize suite-level aggregation and cleanup.
     * This method may be called multiple times for different suites in the
     * same test run.</p>
     * 
     ...
     * 
     * <p><b>Framework Hook:</b> This method is invoked BY the test framework
     * (TestNG, JUnit, etc.) when a test begins, NOT called directly by test code.
     * Implementations should track the current test context for adding
     ...
     * 
     * <p><b>Framework Hook:</b> This method is invoked BY the test framework
     * (TestNG, JUnit, etc.) when a test passes, NOT called directly by test code.
     * Implementations should record the test result and attach any
     * collected artifacts/metrics to the test case.</p>
     * 
     * <p><b>Note:</b> In many implementations, actual result recording happens
     * in the framework-specific listener method (e.g., {@code onTestSuccess(ITestResult)}),
     * while this method serves as a hook for custom logic like logging or notifications.</p>
     * 
     ...
     * 
     * <p><b>Framework Hook:</b> This method is invoked BY the test framework
     * (TestNG, JUnit, etc.) when a test fails, NOT called directly by test code.
     * Implementations should record the test result, including error
     * information, and attach any collected artifacts/metrics to the test case.</p>
     * 
     * <p><b>Note:</b> In many implementations, actual result recording happens
     * in the framework-specific listener method (e.g., {@code onTestFailure(ITestResult)}),
     * while this method serves as a hook for custom logic like logging or notifications.</p>
     * 
     ...
     * 
     * <p><b>Framework Hook:</b> This method is invoked BY the test framework
     * (TestNG, JUnit, etc.) when a test is skipped, NOT called directly by test code.
     * Implementations should record the test as skipped. Artifacts and
     * metrics may still be attached to skipped tests.</p>
     * 
     * <p><b>Note:</b> In many implementations, actual result recording happens
     * in the framework-specific listener method (e.g., {@code onTestSkipped(ITestResult)}),
     * while this method serves as a hook for custom logic like logging or notifications.</p>
     * 
     ...
     * 
     * <p><b>Test Code API:</b> This method is called BY test code during test execution
     * to attach screenshots, logs, videos, HTTP data, etc. to the currently running test.</p>
     * 
     * <p>Artifacts include screenshots, logs, videos, HTTP request/response data,
     * and other files generated during test execution. The artifact will be
     * associated with the specified test and included in the final TestRun report.</p>
     * 
     * <p>Thread-safe: can be called from different threads for different tests
     * during parallel execution.</p>
     * 
     * <p><b>Common Artifact Types:</b></p>
     * <ul>
     ...
     * 
     * <p><b>Test Code API:</b> This method is called BY test code during test execution
     * to record performance measurements like API response times, page load times, etc.</p>
     * 
     * <p>Metrics capture quantitative measurements during test execution such as
     ...
     * during parallel execution.</p>
     * 
     * <p><b>Common Metric Examples:</b></p>
     * <ul>
